<div class="card rounded-0">
	<div class="card-body">
		...
		<div class="form-group row row-nm mb-0" id="filter">
			<div class="col-sm-3">
				<input name="keyword" type="text" class="form-control mb-3" placeholder="Tên, mã, điện thoại hoặc email..." value="{$out.keyword|default:''}">
			</div>
			<div class="col-sm-3">
				<select name="tt" class="form-control mb-3">{$out.select_tt}</select>
			</div>
			<div class="col-sm-2">
...
function filter(){
	...
	var tt = $("#filter select[name=tt]").val();
	
	var url = "./?site=allcustomer";
	if(tt&&tt!=0) url = url+"&tt="+tt;
