<div class="body-head">
	<div class="row">
		<div class="col-md-8">
			<h1>
				<i class="fa fa-bars fa-fw"></i> Thống kê chi tiết các cuộc gọi
			</h1>
		</div>
	</div>
</div>
	...
	<table
		...
		<tbody>
			...
			<tr id="field{$data.id}">
				...
				<td>{$data.phone}</td>
				<td>{$data.email}</td>
				<td style="word-break: break-all; width: 50%"><a href="{$data.url}" target="_blank">{$data.url}</a></td>
				<td>{$data.ismobile}</td>
				<td>{$data.location}</td>
...
function filter(){
    ...
    var to = $.trim($("#to").val());
    var url = "?mod=home&site=accesslogusers";
