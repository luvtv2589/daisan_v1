...
class Media extends Admin {
    ...
    function fetch_media($folder=0, $page=1, $action=null, $limit=48, $type='image') {
        ...
        $sql = "SELECT id,name FROM media WHERE name IS NOT NULL";
    ...
    function get_folder_list($active=0) {
        ...
        while ($item = $stmt->fetch()) {
            if (($active == 0 && $item['id'] == $default_folder) || ($active != 0 && $active == $item['id'])) {
                $result .= '<li role="presentation" class="active"><a href="javascript:void(0);" onclick="ChangeFolder('.$item['id'].');"><i class="fa fa-folder-o fa-fw"></i> '.$item['name'].'</a></li>';
            }else {
                $result .= '<li role="presentation"><a href="javascript:void(0);" onclick="ChangeFolder('.$item['id'].');"><i class="fa fa-folder-o fa-fw"></i> '.$item['name'].'</a></li>';
            }
        }
        return $result;
    }
