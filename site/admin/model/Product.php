...
class Product extends Admin {
    ...
    function index() {
        ...
        while ($item = $stmt->fetch()) {
            ...
            $item ['avatar'] = URL_IMAGE_S3 . $this->product->get_folder_img_s3($item['id']) .'270x270/' . @$a_images[0];
            // $item ['avatar'] = $this->img->get_image($this->product->get_folder_img_s3($item['id']), @$a_images[0]);
...
//                    $file_data = strtolower(pathinfo($this->page->get_folder_img_upload($old_page_id).$imgname, PATHINFO_DIRNAME));
                    $type = strtolower(pathinfo($this->page->get_folder_img_upload($old_page_id).$imgname, PATHINFO_EXTENSION));
                    $filename = strtolower(pathinfo($this->page->get_folder_img_upload($old_page_id).$imgname, PATHINFO_FILENAME));
