...
.list-online-agents ul li {
	padding: 5px;
	border: 1px solid #dddd;
	border-top: none;
}

.list-online-agents ul li:hover {
	cursor: pointer;
}
...
<div class="" style="margin-top: 0px">
	<div class="row">
		{*
		<div class="col-sm-12">
			*} {* <input class="form-control" name="search-agent"
				id="search-agent" value="" placeholder="Tìm kiếm tên gian hàng"
				type="text" style="border-radius: 20px; margin-right: 5px;" />*} {*
		</div>
		*}
		<!-- <div class="col-sm-6">
		...
		<div class="col-sm-12">
			<div class="row">
				<div class="col-sm-5"
					style="position: absolute; top: 75px; left: 40px; background-color: #fff; border-bottom: 4px solid #ffc220; width: 404px; height: 569px; z-index: 99;">
					<table class="table table-responsive table-no-border">
						<tr>
							<td class="list-online-agents">
								<ul>
									{foreach from=$address key=k item=data}
									<li id="location-{$data.id}" class="online-location"
										data-id="{$data.id}" data-lat="{$data.lat}"
										data-lng="{$data.lng}" data-text="#{$data.name}"><span
										class="text-aqua"> <i class="fa fa-circle"></i>
									</span> <strong>{$data.name_page}</strong><br> <strong
										style="color: blue">{$data.name_add}</strong><br> <span
										class="address">{$data.address}</span></li> {/foreach}
								</ul>
							</td>
						</tr>
					</table>
...
<script
	src="{$arg.stylesheet}plugins/jquery-autocomplete/jquery.autocomplete.min.js"></script>
<script
...
<script>
			...
			function() {
				...
				$('#search-agent').submit(
						function() {
							...
							$("tr td.list-online-agents ul li").filter(
									function() {
										$(this).toggle(
	...
	function searchAddress() {
		...
		$
				.post('?mod=home&site=ajax_mapHome', data)
				.done(
						function(res) {
	...
	function searchBox() {
		var searchBox = new google.maps.places.Autocomplete(document
