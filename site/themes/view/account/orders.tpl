	...
	<table class="table table-bordered">
		...
		<tr id="FID{$data.id}">
			...
			<td class="text-right">
				...
				<button type="button" class="btn btn-light btn-sm" onclick="ChangeStatus({$data.id}, 4);"><i class="fa fa-remove fa-fw"></i></button>
...
function filter(){
	...
	var url = "?mod=account&site=orders";
...
function LoadDetail(id){
	...
	data['id'] = id;
	loading();
	$.post('?mod=account&site=order_detail', data).done(function(e){
...
function LoadForm(id){
	...
	loading();
	$.post('?mod=account&site=orders', data).done(function(e){
