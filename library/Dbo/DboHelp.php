...
class DboHelp{
	...
	function get_select_location($active=0, $parent=0, $default='Lựa chọn') {
		...
		$sql = "SELECT Id, Name, Prefix, MapIp FROM locations a WHERE Status=1  AND Parent=$parent ORDER BY Featured DESC,Sort DESC,Name";
		...
		while ($item = $stmt->fetch()) {
            if(!empty($item['MapIp'])) {
                $latLng = explode(',', $item['MapIp']);
                $lat = !empty($latLng[0]) ? $latLng[0] : 0;
                $lng = !empty($latLng[1]) ? $latLng[1] : 0;
            }

			if ($item['Id']==$active) $result .= '<option value="'.$item ['Id'].'" data-lat="'.$lat.'" data-lng="'.$lng.'" selected>';
			else $result .= '<option value="'.$item['Id'].'"data-lat="'.$lat.'" data-lng="'.$lng.'" >';
			$result .= $item ['Name'];
			$result .= '</option>';
		}
		return $result;
